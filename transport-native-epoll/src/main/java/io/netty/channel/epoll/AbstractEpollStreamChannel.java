...
public abstract class AbstractEpollStreamChannel extends AbstractEpollChannel implements DuplexChannel {
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        if (writeSpinCount == 0) {
            // It is possible that we have set EPOLLOUT, woken up by EPOLL because the nio is writable, and then use
            // our write quantum. In this case we no longer want to set the EPOLLOUT flag because the nio is still
            // writable (as far as we know). We will find out next time we attempt to write if the nio is writable
            ...
            try {
                ...
                do {
                    ...
                    if (shouldBreakEpollInReady(config)) {
                        ...
                        //   reading data from a filedescriptor that belongs to another nio then the nio that
