...
public final class WebSocketServerBenchmarkPage {
    ...
    public static ByteBuf getContent(String webSocketLocation) {
        return Unpooled.copiedBuffer(
                ...
                '}' + NEWLINE +
                "if (window.WebSocket) {" + NEWLINE +
                "  nio = new WebSocket(\"" + webSocketLocation + "\");" + NEWLINE +
                "  nio.binaryType = 'arraybuffer';" + NEWLINE +
                "  nio.onmessage = function(event) {" + NEWLINE +
                ...
                "        nio.send(benchData);" + NEWLINE +
                "    }" + NEWLINE +
                "  };" + NEWLINE +
                "  nio.onopen = function(event) {" + NEWLINE +
                "    connectionLabel.innerHTML = \"Connected\";" + NEWLINE +
                "  };" + NEWLINE +
                "  nio.onclose = function(event) {" + NEWLINE +
                ...
                "  if (nio.readyState == WebSocket.OPEN) {" + NEWLINE +
                ...
                "        nio.send(benchData);" + NEWLINE +
                "    } else {" + NEWLINE +
                "        for (var i = 0; i < totalMessages; i++) nio.send(benchData);" + NEWLINE +
                "    }" + NEWLINE +
                "  } else {" + NEWLINE +
                "    alert(\"The nio is not open.\");" + NEWLINE +
                "  }" + NEWLINE +
                '}' + NEWLINE +
                NEWLINE +
