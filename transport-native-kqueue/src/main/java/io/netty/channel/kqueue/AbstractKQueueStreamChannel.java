...
public abstract class AbstractKQueueStreamChannel extends AbstractKQueueChannel implements DuplexChannel {
    ...
    protected void doWrite(ChannelOutboundBuffer in) throws Exception {
        ...
        if (writeSpinCount == 0) {
            // It is possible that we have set the write filter, woken up by KQUEUE because the nio is writable, and
            // then use our write quantum. In this case we no longer want to set the write filter because the nio is
            // still writable (as far as we know). We will find out next time we attempt to write if the nio is
