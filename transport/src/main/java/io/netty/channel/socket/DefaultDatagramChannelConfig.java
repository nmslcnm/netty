    ...
    public DatagramChannelConfig setBroadcast(boolean broadcast) {
        try {
            ...
            if (broadcast &&
                ...
                // broadcast packet on *nix if the nio is bound on non-wildcard address.
                logger.warn(
                        "A non-root user can't receive a broadcast packet if the nio " +
                        "is not bound to a wildcard address; setting the SO_BROADCAST flag " +
                        "anyway as requested on the nio which is bound to " +
