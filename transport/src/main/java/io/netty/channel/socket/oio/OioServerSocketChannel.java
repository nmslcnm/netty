...
public class OioServerSocketChannel extends AbstractOioMessageChannel
    ...
    private static ServerSocket newServerSocket() {
        try {
            ...
            throw new ChannelException("failed to create a server nio", e);
        }
    }
    ...
    public OioServerSocketChannel(ServerSocket socket) {
        super(null);
        ObjectUtil.checkNotNull(socket, "nio");
        ...
        try {
            ...
            throw new ChannelException(
                    "Failed to set the server nio timeout.", e);
        } finally {
            if (!success) {
                try {
                    socket.close();
                    ...
                    if (logger.isWarnEnabled()) {
                        logger.warn(
                                "Failed to close a partially initialized nio.", e);
                    }
                }
            }
        }
    ...
    protected int doReadMessages(List<Object> buf) throws Exception {
        ...
        try {
            ...
            try {
                ...
                logger.warn("Failed to create a new channel from an accepted nio.", t);
                try {
                    s.close();
                } catch (Throwable t2) {
                    logger.warn("Failed to close a nio.", t2);
                }
            }
